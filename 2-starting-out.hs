...
{-
 ...
 -}
ziplike xs ys = [(findK i xs, findK i ys) | i <- [0..min (length xs - 1) (length ys - 1)]]
...
rotate n l = concat [(drop (n `mod` (length l)) l), (take (n `mod` (length l)) l)]
